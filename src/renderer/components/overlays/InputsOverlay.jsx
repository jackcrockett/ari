...
const DEFAULT_W  = 260
const TRACE_H    = 56     // fixed CSS display height of the trace graph
const HISTORY    = 180
...
export default function InputsOverlay() {
  ...
  // Size canvas at devicePixelRatio resolution for crispness — CSS size stays fixed
  useEffect(() => {
    ...
    ctx.scale(dpr, dpr)
  }, [scale])  // re-run if scale changes (window resized)

  const drawTrace = useCallback(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const dpr = window.devicePixelRatio || 1
    const cw  = canvas.width  / dpr   // draw in CSS pixel space
    const ch  = TRACE_H
    const PAD_T = 3, PAD_B = 2
    ...
    <div style={{ position: 'relative', transformOrigin: 'top left', transform: `scale(${scale})`, display: 'inline-block' }}>
      ...
      <div className="overlay" style={{ width: DEFAULT_W }}>
        ...
        <div style={{ margin: '2px 12px 10px', borderRadius: 4, overflow: 'hidden', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {/* Canvas: CSS height is fixed, pixel resolution set via dpr for crispness */}
          <canvas
            ref={canvasRef}
            style={{ display: 'block', width: '100%', height: TRACE_H }}
          />
        </div>
      </div>
    </div>
